...
1. **Inline-cite every load-bearing claim with its SHA-pinned permalink, in the source forge's grammar** — the link the reference gives for that fact (versions, defaults, signatures, deprecations, behavior a user could get wrong by guessing), for example on github.com `https://github.com/<owner>/<repo>/blob/<sha>/<path>#L<start>-L<end>`. Put the permalink inline, on the claim. Use a bare filename parenthetical (e.g. *(reference-name.md)*) only when the reference genuinely provides no permalink. This inline permalink is what the grounded-citation eval (SELF-AUDIT Check 8) grades.
