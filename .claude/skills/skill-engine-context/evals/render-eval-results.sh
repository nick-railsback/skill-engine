...
# render-eval-results.sh parameterized template for the skill-engine-context
# navigator's eval-results renderer.
#
# This is a TEMPLATE. Replace placeholders before use:
# skill-engine           - your domain stem (used in output header)
#
...
#     2. Per-persona pass rates AND overall pass rate. Runs recorded as
...
#        newly flickering. Entries voting "error" on either side are listed
#        as [ERRORED] and excluded from the comparison.
#
# Determinism:
...
# Outputs: "<vote>:<flicker>" on stdout (vote = pass|fail|error,
# flicker = yes|no). "error" runs are infra outcomes: they never count
# toward the pass-vs-fail vote, and an entry votes "error" only when NO
# run produced a real verdict. The colon separator avoids the
# editor-fragility of a literal tab.
decide() {
  local outcomes="$1"
  ...
  local vote flicker
  if [ "$pass" -eq 0 ] && [ "$fail" -eq 0 ] && [ "$err" -gt 0 ]; then
    vote=error
...
echo "=== skill-engine-context eval results ==="
...
# `runs` is the rest-field: read packs fields 4..N into it US-separated,
# so the loop handles any runs_per_query — N-generic like emit_records
# and aggregate() (a hardcoded r1 r2 r3 read would glue runs 3..N into
# one token that matches no outcome and silently drops them from the vote).
# shellcheck disable=SC2034 # persona consumes its US-delimited field so runs lands correctly; the per-entry lines don't print it
while IFS="$US" read -r query expected persona runs; do
  [ -z "$query" ] && continue
  outcomes=$(printf '%s' "$runs" | tr "$US" ',')
  decision=$(decide "$outcomes")
  vote="${decision%%:*}"
  flicker="${decision##*:}"
  if [ "$flicker" = "yes" ]; then
    printf '  [%s] [FLICKER] %s -> %s | %s\n' "$vote" "$query" "$expected" "$outcomes"
  else
    printf '  [%s] %s -> %s | %s\n' "$vote" "$query" "$expected" "$outcomes"
  fi
done < "$TMP_RECORDS"
...
if [ -n "$BASELINE_PATH" ]; then
  ...
  awk -v US="$US" '
    BEGIN{FS=US}
    # Both helpers read run fields 4..NF of the record in flight directly,
    # so they stay N-generic alongside emit_records and aggregate().
    function vote_of(    p, f, i) {
      # Mirrors decide(): error runs carry no verdict; an all-error entry
      # votes "error" and is excluded from delta comparison below.
      p = 0; f = 0
      for (i = 4; i <= NF; i++) {
        if ($i == "pass") p++
        else if ($i == "fail") f++
      }
      if (p == 0 && f == 0) return "error"
      return (p > f) ? "pass" : "fail"
    }
    function flicker_of(    p, f, i) {
      p = 0; f = 0
      for (i = 4; i <= NF; i++) {
        if ($i == "pass") p++
        else if ($i == "fail") f++
    ...
    }
    {
      ...
      if (v == "error" || base_vote[$1] == "error") {
        printf "  [ERRORED] %s -> %s (excluded from delta; rerun after fixing the runner)\n", $1, $2
        next
      }
