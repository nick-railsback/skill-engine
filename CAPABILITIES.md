...
Skill-engine sits on top of Anthropic's [Agent Skills guide](https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf)
and ships everything the spec deliberately leaves for an operator to build. The
spec defines *what a skill is*. This document inventories *what an engine for
maintaining skills at scale must additionally do*, and how skill-engine does it.

If you came here from the project README, the five features named in *"What else
is in the box"* each have a section below. If you came here from a persona page,
the feature called out in *"Why it works"* has its own anchor. Both reader paths
should land cleanly.

## Contents

- [How it gets built](#how-it-gets-built) — DISCOVER, source kinds, scaffolding
- [How it stays accurate](#how-it-stays-accurate) — drift detection, REFRESH, SHA-pinning
- [How it knows it's still right](#how-it-knows-its-still-right) — SELF-AUDIT, invariants
- [How it scales](#how-it-scales) — multi-source synthesis, monorepo slices
- [How it composes](#how-it-composes) — multiple contextualizers at once
- [How you evaluate it](#how-you-evaluate-it) — the eval harness
- [How it handles failure](#how-it-handles-failure) — archived sources, missing branches
- [How it's distributed](#how-its-distributed) — plugin install, CLI installer, versioning
- [How human review fits](#how-human-review-fits) — reviewer-in-the-loop discipline
...
See also: [How it scales](#how-it-scales), [How human review fits](#how-human-review-fits).
...
A contextualizer that works against five small repos is a different artifact
than one that works against hundreds of heterogeneous sources across a
monorepo. Skill-engine treats scale as a first-class architectural concern.

### Multi-source synthesis

The load-bearing capability. Register N sources — git repositories, external
docs, local paths, monorepo slices — and skill-engine emits a single
navigator skill that reasons across all of them. The synthesis is built at
discover time: the model reads each source, decides what matters, writes
references that cite their origins by `(source_id, path, sha)` triple, and
the navigator's catalog maps each reference back to the sources it draws
from. Reasoning across that graph happens at query time — when Claude
answers a question that touches four sources at once, it holds the tensions
...
sequence. Not because the engine works on any single source — because it
...
See also: [How it composes](#how-it-composes) (for multi-contextualizer
loading, which is a different scaling axis), [How it gets built](#how-it-gets-built)
...
Skill-engine produces contextualizers that are themselves project skills —
which means they obey Anthropic's Agent Skills loading semantics. Multiple
contextualizers can be active in a session simultaneously, and Claude
activates only the ones whose navigator descriptions match the current
intent. This is the property the Solutions Engineer persona depends on.

### Why per-domain contextualizers compose cleanly
...
descriptions for activation precision).
...
See also: [How it composes](#how-it-composes) (for the WHEN-not-WHAT
discipline the eval harness measures).
