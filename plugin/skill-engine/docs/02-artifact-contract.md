...
**`archived`** — a boolean the user sets by hand, or REFRESH proposes. As of v0.9.0, REFRESH's Phase 0.5 reads the forge's own archived flag for every in-scope `git-managed` source before any HEAD probe, dispatched by the registered URL's host: `gh api repos/<owner>/<repo>` on github.com, a read-only `GET /api/v4/projects/<url-encoded path>` on GitLab, `GH_HOST=<host> gh api repos/<owner>/<repo>` on any other host `gh` resolves, and no call at all for anything else — which stays `unknown`. The engine passes no token of its own; `gh` uses whatever ambient credentials it already has, so this is not an unauthenticated read. Any answer that is not 2xx is `unknown`, never `false`. A `true` flag is staged as `archived: true` into the *proposed* `source-paths.json` for the reviewer to accept at `apply` — REFRESH never flips the live file. The dispatch table and the unknown-vs-failed accounting live in [`refresh/references/drift-detection-and-phases.md`](../skills/refresh/references/drift-detection-and-phases.md) § Phase 0.5. Once `archived: true` is live, REFRESH and DISCOVER skip the source's crawl pass, and any lifecycle sweep proposal treats citations to this source the same as it would for a `removed` state — `cut_block` by default. Defaults to `false` when absent.
...
|---|---|
| **Contract frontmatter fields only (`name`, `description`, optional `paths:`)** | Cross-platform navigator loading (some platforms drop unknown fields, others reject them). `paths:` is Claude Code-scoped and outside that guarantee; see [Frontmatter fields](#frontmatter-fields) |
