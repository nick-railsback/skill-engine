...
**Validation rules** (the schema `monorepo-config.json` must satisfy — enforced by the `monorepo-config` check the shipped `verify.sh` runs between its source-paths-shape and source-entries checks (template source: `plugin/skill-engine/engine-bootstrap-templates/verify.sh`). The `monorepo-coverage` *heuristic* (Check 6) is a separate concern: it warns when a workspace member is uncited, not whether the config file itself is well-formed):
...
- Slice `paths` are **`git sparse-checkout --no-cone` patterns, i.e. gitignore
  syntax** — not pathspec globs. A pattern matching a directory pulls in that
  directory's whole subtree (`packages/billing/*` carries
  `packages/billing/src/a.ts`), a slashless pattern matches at any depth
  (`*.proto` carries `proto/sub/deep.proto`), and a leading `!` carves a
  subtree back out. `slice_drift.py` asks the real sparse-checkout machinery
  what a pattern set selects rather than reimplementing these rules, so what
  counts as drift for a slice is by construction what its checkout carries.
  Matching with pathspec semantics instead (`:(glob)`, fnmatch with
  `FNM_PATHNAME`) diverges on exactly those three shapes, and a slice whose
  changed file falls in the gap is checked out once and then never refreshed
  again, silently.
...
**Check 6's own root detection is separate from `monorepo-config.json`.** It resolves a `git-managed` source's tree from the engine's clone cache (`$SKILL_ENGINE_CACHE_ROOT/git-managed/<id>-<sha>/`, where the suffix must be bare hex so a sibling id or an in-flight `.tmp` staging clone is not mistaken for the source's own tree) rather than skipping the source outright, and reports `[N/A]` naming the reason when no cache directory exists yet instead of silently passing. With no cache directory there is one fallback: a `git-managed` source whose `path` names an **absolute** directory that exists is read from there, which is how a vendored or already-checked-out upstream is covered without a clone. The absolute requirement is load-bearing rather than stylistic — `verify.sh` never `cd`s, the schema does not constrain `path` on a `git-managed` entry, and a relative one would therefore resolve against whatever directory the run was launched from, letting the check enumerate the maintainer's own tree as if it were the upstream. A relative `path` gets the same `[N/A]` as no `path` at all. Check 8's catalog-density floor resolves its tree the same way, for the same reason. Workspace members are enumerated from the default root list `packages apps libs crates services modules cmd internal pkg`, or from a source's own `workspace_roots` array in `source-paths.json` when set, which replaces — not augments — the defaults. A slice source — one carrying `slice_of`/`slice_id`/`slice_paths` in `source-paths.json` — is checked the same way, using its own `slice_paths` in place of `workspace_roots`, resolved against the slice's own sparse clone (installed under the slice's own derived source id by `cache-git.sh sparse-clone`) rather than its parent's, whose url every sibling slice shares and which an applied parent's exclusion from crawling stops advancing. "The same way `workspace_roots` does" is literal on both counts: **every** `slice_paths` entry is read, not only the first, and the citation test is anchored on the slice's own **path** — the literal prefix of each pattern, before its first glob character — never on its bare `slice_id`, for the reason the workspace-member test is anchored on `<root>/<member>`: an id like `api` otherwise matches inside a cited sibling's `web-api`, and an id like `billing` matches the word "billing" in any reference's prose. A parent with applied slices is excluded from member enumeration here too, so one directory cannot receive two opposite verdicts in a single run. `slice_paths` is the `source-paths.json` entry's field this check reads; it is related to but distinct from `monorepos[].slices[].paths`, the config file's own field — DISCOVER derives one from the other when staging slice entries (see `discover/references/cache-and-clone.md`).
...
The adapter is **read** by the `maintenance-agent.md.template` (find at `plugin/skill-engine/engine-bootstrap-templates/maintenance-agent.md.template` in your installed plugin, or at <https://github.com/nick-railsback/skill-engine/blob/main/plugin/skill-engine/engine-bootstrap-templates/maintenance-agent.md.template>). The adapter's config schema is machine-validated by the consumer-stamped `verify.sh`'s `monorepo-config` check (§7.3); the `monorepo-coverage` heuristic (Check 6) is a separate concern, warning on uncited workspace members rather than on config well-formedness. The adapter is **bootstrapped** by `bootstrap-monorepo-config.sh.template`. The adapter is **shaped** by per-slice `CLAUDE.md` files the maintainer authors in their monorepo.
