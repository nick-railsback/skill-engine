...
- `probe_budget` (document-root, optional integer ≥ 1). When set, REFRESH carries at most N probe-promoted sources per session into re-read scoping, ordered by descending `importance` (absent ⇒ 3), with ties broken by oldest `lifecycle.last_checked` first (missing ⇒ `1970-01-01T00:00:00Z`, so never-probed sources sort to the front), with further ties broken by ascending source `id`. Both tie-breakers name fields of `source-paths.json` itself, which is what the ordering recipe in `refresh/references/drift-detection-and-phases.md` § Promotion and ordering sorts — `probed_at` belongs to `.engine-stats.json`'s `sources_probed` telemetry and is not reachable from that sort, and `path` is not what it orders by. Sources beyond the budget are explicitly skipped (not silently dropped) — REFRESH renders `"M of K sources skipped this session due to probe_budget=N (next-eligible: <list>)"`. The fetch cost of `freshness_policy: fetch-and-compare` is NOT counted against `probe_budget` — only the re-read step is budgeted (`probe_budget` controls model-token cost, not network cost), so every in-scope source is probed regardless. Validation: `probe_budget` MUST be a JSON integer ≥ 1 when present — enforced by the schema (`source-paths.schema.json`); values 0, negative integers, and non-integer types fail loudly at REFRESH activation naming both the field and the offending value. Absent budget: every promoted source proceeds to re-read, and all sources are probed every session either way.
