...
| **SELF-AUDIT** | Five drift checks: stale-date, broken-URL, long-untouched, catalog-vs-content, cross-reference-vs-content. Read-only by default; surfaces findings then offers a per-run opt-in propose-approve gate for the deterministic ones (stale-date, catalog drift). The other three remain advisory | Every 2-4 weeks; pair with DISCOVER on the same day for a quarterly rhythm |
