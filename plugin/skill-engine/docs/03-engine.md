...
| **SELF-AUDIT** | Six drift checks: stale-date, broken-URL, long-untouched, catalog-vs-content, cross-reference-vs-content, review-state staleness. Read-only by default; surfaces findings then offers a per-run opt-in propose-approve gate for the deterministic ones (stale-date, catalog drift, review-state staleness). The other three remain advisory | Every 2-4 weeks; pair with DISCOVER on the same day for a quarterly rhythm |
