...
* **Plugin install (recommended).** Install `plugin/skill-engine/` via the Claude Code plugin marketplace; the eight skills (`using-skill-engine`, `refresh`, `new-reference`, `discover`, `status`, `self-audit`, `engine-bootstrap`, `clean-cache`) are invokable as `/skill-engine:<skill>`.
...
| **SELF-AUDIT** | Seven drift checks: stale-date, broken-URL, long-untouched, catalog-vs-content, cross-reference-vs-content, review-state staleness, paragraph→permalink density. Read-only by default; surfaces findings then offers a per-run opt-in propose-approve gate for the deterministic ones (stale-date, catalog drift, review-state staleness). The other four remain advisory | Every 2-4 weeks; pair with DISCOVER on the same day for a quarterly rhythm |
...
4. **Catalog row description vs reference content.** For each catalog row in the navigator, diff its one-line description against the reference's frontmatter `description:` field — the canonical, model-readable statement of what the reference is for. If the reference also carries a body section titled `## When to Use This Reference` (the legacy top-of-file framing in [02-artifact-contract.md](02-artifact-contract.md)), treat substantive divergence between the catalog row, the frontmatter description, and that section as the same finding. One-token-different wording is fine; a catalog row that no longer matches the reference's stated purpose is the case to find. References with no `## When to Use This Reference` section are not flagged on that basis alone (DISCOVER is goal-given and emits whatever body shape the source rewards); the frontmatter `description:` is the contract.
...
2. **Validate.** Dispatch the `worker-verify` worker (see [`plugin/skill-engine/agents/worker-verify.md`](https://github.com/nick-railsback/skill-engine/blob/main/plugin/skill-engine/agents/worker-verify.md)) against the sandbox. All three pre-fixture-harness checks (no-frontmatter, catalog bijection, `./verify.sh`) must pass — same pre-approval contract REFRESH uses. (Checksum fixture refresh and full test suite are pre-fixture-harness aspirational; see the [Pre-approval validation](#pre-approval-validation-the-load-bearing-contract) section above.)
...
For **judgment-required findings** (Checks 2, 3, 5), the workflow prints a one-line recommendation per finding and exits without drafting any mutation. Sample recommendations:
...
**Why this scope.** Checks 1 and 4 have a single correct mutation each (refresh-the-date, sync-the-row); a drafted fix is right ~100% of the time and the propose/approve loop reads as a sanity confirmation rather than a content judgment. Checks 2/3/5 require knowing the user's intent (which broken URL replacement is right? what is the new routing?); a drafted fix here would frequently be wrong and would erode trust in the approve/defer prompt elsewhere. v1 keeps the propose surface narrow on purpose.
...
SELF-AUDIT also does not validate the artifact contract. The four reference invariants, frontmatter rules, filename conventions, and other shape-of-the-corpus checks are owned by `verify.sh` (the stamped per-contextualizer audit). If the auditor notices a contract-side question (e.g., "doctrine says no frontmatter but references have it"), that question belongs in the session-reflection's `## Template ambiguities` block, not the findings list. The seven drift checks above are the entire scope of SELF-AUDIT's findings surface; broadening it would conflate framing drift with invariant compliance and erode the per-lens separation that makes the three workflows actionable. Check 7 (paragraph→permalink density) sits orthogonally to Checks 1–6: the others detect framing drift across several axes (stale dates, broken URLs, long-untouched references, catalog rows, cross-reference rules, review-state), while Check 7 measures structural-honesty density on the references corpus — same workflow, adjacent lens.
