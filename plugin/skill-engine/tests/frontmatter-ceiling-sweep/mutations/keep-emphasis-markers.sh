...
# sweep stops deleting `*` before matching, so `**only** the two` no longer
# reads as one phrase. Backticks are still deleted, so only the emphasis
# fixture can turn this red.
#
