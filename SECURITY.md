...
`skill-engine` is pre-1.0. Only the latest pre-1.0 minor line (currently `0.8.x`) receives security fixes; prior minor lines are end-of-life. When `1.0` ships, this table will expand to cover the most recent minor line plus a backport window.
