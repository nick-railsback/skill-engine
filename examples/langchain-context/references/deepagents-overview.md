...
The repo lives at [langchain-ai/deepagents](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066). The root [`README.md`](https://github.com/langchain-ai/langchain/blob/7bb4130c7d460f14ec6391805cb47bf01637b5c5/README.md) of `langchain-ai/langchain` calls it out in a TIP block: "higher-level package built on LangChain for agents that have built-in capabilities for common usage patterns" — which is the right framing. If `create_agent` is "the agent kernel," Deep Agents is "the agent distro."

## Repo layout

A monorepo of independently versioned PyPI packages under `libs/`:

- [`libs/deepagents/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents) — publishes `deepagents` (v0.6.4). The SDK. Public surface re-exported from [`deepagents/__init__.py`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/deepagents/__init__.py).
- [`libs/cli/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/cli) — publishes `deepagents-cli` (v0.1.2). **As of 0.1.0 this package contains only deployment subcommands** (`init`, `dev`, `deploy`) for bundling and shipping agents to LangGraph Platform. The interactive coding REPL that used to live here moved to `deepagents-code` (see below). Install with `uv tool install deepagents-cli`.
- [`libs/code/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/code) — publishes `deepagents-code` (v0.1.6). The interactive terminal coding agent — a TUI similar in shape to Claude Code or Cursor, powered by any tool-calling LLM. Install via `curl -LsSf https://langch.in/dcode | bash`, run as `dcode`. Adds streaming TUI, conversation resume, web search, remote sandboxes (LangSmith / AgentCore / Daytona / Modal / Runloop), persistent memory, custom skills, headless mode, and HITL tool gating on top of the SDK.
- [`libs/acp/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/acp) — Agent Context Protocol support.
- [`libs/evals/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/evals) — evaluation suite and Harbor integration.
- [`libs/partners/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/partners) — sandbox/runtime integrations: `daytona`, `modal`, `quickjs`, `runloop`. These wrap external sandboxes that the `execute` tool delegates to so untrusted shell commands don't run against the host.

## Public Python surface

The SDK is small. The headline export from [`deepagents/__init__.py`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/deepagents/__init__.py):
...
Tools that touch the world — filesystem, shell — are pluggable backends in [`deepagents/backends/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/deepagents/backends). The `protocol.py` module defines the interface; concrete implementations include `local_shell.py` (runs on the host), `sandbox.py` (delegates to a remote sandbox), `composite.py` (combines multiple), `langsmith.py` (records events to LangSmith for inspection), `filesystem.py` (`FilesystemBackend` — reads and writes the host filesystem directly), `context_hub.py` (`ContextHubBackend` — persists files to a LangSmith Hub agent repo), and `state.py` and `store.py` (graph-state-backed virtual filesystem); `utils.py` holds shared helpers the backends and composite router share. The four `libs/partners/` packages — Daytona, Modal, QuickJS, Runloop — each provide a sandbox backend.

This is the practical answer to "but won't a model with shell access destroy my machine" — the default backend can be a virtual in-state filesystem with no real shell, or a remote sandbox. The README's security note is blunt: "Deep Agents follows a 'trust the LLM' model. The agent can do anything its tools allow. Enforce boundaries at the tool/sandbox level, not by expecting the model to self-police." See also [`libs/deepagents/THREAT_MODEL.md`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/THREAT_MODEL.md).

## How Deep Agents relates to the rest of the ecosystem

The dependency chain is `deepagents → langchain (v1) → langgraph → langchain-core`. Specifically the [pyproject](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/pyproject.toml) pins `langchain >=1.3.2,<2.0.0`, `langchain-core >=1.4.0,<2.0.0`, `langsmith >=0.8.3`, plus `langchain-anthropic` and `langchain-google-genai` as default model providers.
...
If a user already has a `create_agent` setup and wants Deep Agents' filesystem or sub-agent capabilities, they can import the middleware classes (`FilesystemMiddleware`, `SubAgentMiddleware` — both re-exported from [`deepagents/__init__.py`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/deepagents/__init__.py)) and add them to their own middleware list rather than swapping over to `create_deep_agent`. The packages compose.

The compiled output is a LangGraph graph (the [`README.md`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/README.md) calls this out: "LangGraph Native — `create_deep_agent` returns a compiled LangGraph graph"), so streaming, persistence (checkpointers), and Studio inspection all work the same way they do for any other LangGraph agent. See [`langgraph-overview.md`](langgraph-overview.md) for the LangGraph side.
...
- **Pinned model providers.** `langchain-anthropic` and `langchain-google-genai` are *required* dependencies in [`pyproject.toml`](https://github.com/langchain-ai/deepagents/blob/e298206407e38a0d7bdb073d945b5ab19f401066/libs/deepagents/pyproject.toml), even if the user only uses OpenAI. They're loaded eagerly because the default profiles target them. This is unusual for a LangChain package — most expect `langchain-<provider>` to be installed only when needed.
...
- The [`examples/`](https://github.com/langchain-ai/deepagents/tree/e298206407e38a0d7bdb073d945b5ab19f401066/examples) directory has runnable agents and patterns.
