...
Or, for headers, auth, custom timeouts, you supply an `httpx.AsyncClient`:
...
Server side: `MCPServer.run(transport="sse")` or `MCPServer.sse_app()` for mounting. `mount_path` was removed in v2 — use Starlette's `Mount("/path", app=mcp.sse_app())` which sets `root_path` in the ASGI scope and is read automatically by `SseServerTransport`.

## Websocket

`mcp.client.websocket` and `mcp.server.websocket` provide a websocket transport. Requires the `[ws]` extra (`pip install mcp[ws]`). Used much less commonly than the HTTP transports — it's primarily useful when you have an existing websocket pipeline you want to route MCP through.
...
It auto-enables when `host` is `127.0.0.1`, `localhost`, or `::1` to protect dev servers from DNS-rebinding attacks. Pass `transport_security=` to `mcp.run()` / `streamable_http_app()` / `sse_app()` to override. For production with reverse proxies, supply your allowed external hostnames explicitly.

Note: the source includes maintainer TODOs flagging that this middleware is awkward and may be reworked — the API may evolve.

## CORS for browser-based clients

Browser MCP clients need CORS. Add Starlette's `CORSMiddleware` around `streamable_http_app()`:
...
The `mcp-session-id` header is the one to expose so JS clients can capture it for follow-up requests. Per the v1 README (still accurate for transport behavior): the session ID is set on the initial response and must be echoed back on subsequent requests in a stateful session.
